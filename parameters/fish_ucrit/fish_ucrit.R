...
chinook_temp_data = data$salmonid %>% 
  group_by(species) %>% 
  mutate(value = swimspeed_BL/max(swimspeed_BL, na.rm = TRUE)) %>% 
  ungroup() %>% 
...
chinook_predict <- chinook_temp_data %>%
  mutate(predict = predict(chinook_fit,
                              newdata = .))

plotName = ggplot(chinook_predict, aes(x = temperature_C)) +
  theme_classic(base_size = 25) +
  labs(y = "Fraction of Ucrit", x = "Temperature (\u00B0C)") +
